...
class reservaModel extends Model
{
    ...
    public function getTipoCambio($atipo) {
        $sql= 'SELECT TOP 1 cambio FROM tcambio WHERE GETDATE()>=fechad AND GETDATE()<=fechah AND atipoa="'.$atipo.'"';
        $tcambio= $this->_db->consulta($sql);
        if($this->_db->numRows($detUser)>0)
        {
            return $this->_db->fetchAll($tcambio);
        }
        else
        {
            return false;
